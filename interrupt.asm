...
network_rx_as_well:
	...
	; Check for MCP packet
	mov ax, [os_PacketBuffers+14]
	cmp ax, 0xBAAB			; ABBA
	je mcp
	; Check for callback
...
mcp:
	mov rcx, mcp_callback		; RCX stores the callback function address
	mov rsi, rsp			; Copy the current stack pointer to RSI
	sub rsp, 8			; Subtract 8 since we add a 64-bit value to the stack
	mov rdi, rsp			; Copy the 'new' stack pointer to RDI
	movsq				; Flags
...
mcp_callback:
	...
	mov ax, [os_PacketBuffers+16]	; Grab the command ID
	shr ax, 12			; Shift the command bits to AL
	cmp al, 0x00			; 0x00 - Ping
	cmp al, 0x01			; 0x01 - Info
	je mcp_info
	...
	cmp al, 0x07			; 0x07 - Reset
	cmp al, 0x09			; 0x09 - Reboot
	jmp mcp_callback_end

mcp_info:
	mov rdi, os_PacketBuffers+0x8000
	push rdi
	mov rsi, os_PacketBuffers+8
	lodsd
	stosd
	lodsw
	stosw
	...
	lodsw
	stosw
	mov ax, 0xBAAB			; 0xABBA
	stosw
	mov ax, 0x1000			; 0x0010
	stosw
	mov ax, [os_NumCores]
	stosw
	mov eax, [os_CoreSpeed]
	stosw
...
mcp_start:
	...
	call rax			; FIXME - change the stack instead of calling
	jmp mcp_callback_end
